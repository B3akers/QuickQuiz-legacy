...
namespace QuizHouse.Controllers
{
	...
	public class GameOldController : Controller
	{
		...
		public GameOldController(IConfiguration configuration, GamesServiceOld gamesService, IJwtTokenHandler jwtTokenHandler, DatabaseService quizService)
		{
		...
		public IActionResult JoinGame([FromBody] JoinGameParametrs joinGameParametrs)
		{
			...
			{
				if (string.IsNullOrEmpty(joinGameParametrs.Token) || !_jwtTokenHandler.ValidateToken(joinGameParametrs.Token))
		...
		public async Task<IActionResult> TwitchLogin(string code)
		{
			try
			{
				...
				var twitchUsernameToken = _jwtTokenHandler.GenerateToken(new ClaimsIdentity(new Claim[]
				{
