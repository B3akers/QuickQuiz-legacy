...
namespace QuizHouse.Services
{
	public class AccountRepositoryService : IAccountRepository
	{
		...
		{
			var account = new AccountDTO() { Email = email, Username = username, Password = _passwordHasher.Hash(password), CreationTime = DateTimeOffset.UtcNow.ToUnixTimeSeconds(), EmailConfirmed = false, IsAdmin = false, Connections = new List<AccountConnection>() };
			var accounts = _quizService.GetAccountsCollection();
